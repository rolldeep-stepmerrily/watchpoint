 ...
 *
 * NOTE: 영웅 codename pre-validation은 제거됨. `generateMetadata()`에서 API 404를
 * 받으면 `notFound()`를 호출해 커스텀 not-found.tsx + status 404를 반환하는 것으로
 * soft-404를 해결. middleware rewrite(url, {status:404})는 Next.js 기본 404 페이지를
 * 표시해 커스텀 한국어 not-found 페이지를 우회하는 부작용이 있었음.
 */
